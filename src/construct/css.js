...
const PATTERN = new RegExp(
  `([^\\s;}]+|^):(${Object.keys(STATES)
    .map((key) => STATES[key])
    .join("|")})(?=[\\s\\+~,{])`,
  "g"
);

const getClassName = (state) => (props) =>
  `&.${props.transitionClassNames[state]}`;
    ...
    while ((match = PATTERN.exec(chunk))) {
      ...
      if (target === "&") {
        ...
        if (!isAnimatedComponent(Target)) {
          const name =
            Target && Target.constructor ? Target.constructor.name : Target;
          throw new Error(
            `Invalid transition target "${name}". Target must be an AnimatedComponent.`
          );
        }
